...
if st.button('Predict'):
    ...
    # Engineered features — calculated from inputs, not entered by user
    Stock_to_Reorder_ratio = Current_Stock_Qty / (Reorder_Point + 1)
    Revenue_per_day = Total_Revenue_INR / (Days_Since_Last_Sale + 1)
    Profit_margin = Total_Profit_INR / (Total_Revenue_INR + 1)
    Sell_through_rate = Total_Qty_Sold / (Total_Qty_Sold + Current_Stock_Qty + 1)

    input_data = pd.DataFrame([[
        Days_Since_Last_Sale, Current_Stock_Qty, Reorder_Point,
        Lead_Time_Days, Total_Qty_Sold, Total_Revenue_INR,
        Total_Profit_INR, Avg_Unit_Sell_INR, Avg_Unit_Cost_INR,
        Return_Count, Days_To_Expiry,
        Stock_to_Reorder_ratio, Revenue_per_day, Profit_margin, Sell_through_rate
    ]], columns=features)
